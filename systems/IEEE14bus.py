...
class ieee14bus(Network):
    ...
    def _create_buses(self):
        ...
        self.buses = [                                                 
            ...
            Bus(self, id= 9, bus_type=   'PQ', v=1.000, theta=0.0, Sh=19, Sb=100), 
    ...
    def _create_generators(self):
        self.generators = [                                                
            ...
            Generator(id=2, bus=self.buses[1], pb=100, p_input=40.00),     
    ...
    def _create_loads(self):
        self.loads = [                                                           
            Load(id= 1, bus=self.buses[ 1], pb=100, p_input=21.70, q_input=12.70),
            Load(id= 2, bus=self.buses[ 2], pb=100, p_input=94.20, q_input=19.00),
            Load(id= 3, bus=self.buses[ 3], pb=100, p_input=47.80, q_input=-3.90),
            Load(id= 4, bus=self.buses[ 4], pb=100, p_input= 7.60, q_input= 1.60),
            Load(id= 5, bus=self.buses[ 5], pb=100, p_input=11.20, q_input= 7.50),
            Load(id= 6, bus=self.buses[ 8], pb=100, p_input=29.50, q_input=16.60),
            Load(id= 7, bus=self.buses[ 9], pb=100, p_input= 9.00, q_input= 5.80),
            Load(id= 8, bus=self.buses[10], pb=100, p_input= 3.50, q_input= 1.80),
            Load(id= 9, bus=self.buses[11], pb=100, p_input= 6.10, q_input= 1.60),
            Load(id=10, bus=self.buses[12], pb=100, p_input=13.50, q_input= 5.80),
            Load(id=11, bus=self.buses[13], pb=100, p_input=14.90, q_input= 5.00)
        ]
